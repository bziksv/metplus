...
(function () {
				...
				.then(function (data) {
					...
					if (!data || !data.ok) {
						...
						if (data && data.status === 'not_found') {
							setError((data.error || 'Номер не найден.') + ' Перейдите на вкладку «Регистрация».');
							return;
						}
	...
	function setAuthMode(mode) {
		...
		shell.querySelectorAll('.prime-auth-mode-tabs__btn, .prime-auth-mode-tabs button').forEach(function (btn) {
			btn.classList.toggle('is-active', btn.getAttribute('data-mode') === mode);
		});
		shell.querySelectorAll('.auth-mode-panel').forEach(function (panel) {
			panel.classList.toggle('is-active', panel.getAttribute('data-mode-panel') === mode);
		});
	}

	function initAuthMode() {
		document.querySelectorAll('.js-auth-show-register').forEach(function (btn) {
			btn.addEventListener('click', function (e) {
				e.preventDefault();
				setAuthMode('register');
			});
		});
		document.querySelectorAll('.js-auth-show-login').forEach(function (btn) {
			btn.addEventListener('click', function (e) {
				e.preventDefault();
				setAuthMode('login');
			});
	...
	window.primePhoneauthSetMode = setAuthMode;
